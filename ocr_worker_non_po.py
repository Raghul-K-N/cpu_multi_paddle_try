...
# def check_is_non_po_invoice(text_lines: List[str], logger) -> bool:
#     """
...
#     text = " ".join(line.lower() for line in text_lines)
#     is_non_po = all(keyword.lower() in text for keyword in po_keywords1) or any(keyword.lower() in text for keyword in po_keywords2)
#     logger.debug(f"Invoice classified as non-PO: {is_non_po} | fname=CHECK")
#     return is_non_po, text_lines
...
def check_is_exist_invoice(text_line: str, checkindex: int) -> bool:
    """
    Heuristic to determine if the invoice is a non-PO invoice based on presence of keywords.
    """
    po_keywords1 = ['SAP Payment Request', 'To be filled by Requestor', 'Substitute Invoice Form']
    ...
    for page_data in pipeline_output:
        ...
        for i in range(len(texts)):
            ...
            if isinstance(box[0], (int, float)):
                ...
                formatted_box = box
            if matched_len  < 3:
                if check_is_exist_invoice(text, matched_len):
                    matched_len += 1
                    ...
                    matched_len = 0
                    matched_page = None
            print(f"Formatted box for text '{text}': {formatted_box}")
            
            page_items.append([formatted_box, [text, conf]])
        if index == 0:
            print(f"First page raw_boxes: {raw_boxes}")
            print(f"First page formatted_boxes: {[item[0] for item in page_items]}")
        index += 1
        formatted_pages.append(page_items)
        if(matched_page is not None):
            return [formatted_pages[matched_page]]
